...
tuned_parameters = [{'n_neighbors': [5,6,7,8,9,10,20,40]}]
...
def calc_score(X, y, indices, tuned_parameters, estimator):
    ...
    clf_best = KNeighborsClassifier(n_neighbors=clf.best_params_['n_neighbors'])
...
while features:
    ...
    for i in range(0, len(features)):
        ...
        score, model = calc_score(X, y_train, testfs, tuned_parameters, knn)
        if score > maxs:
            maxs = score
            maxmodel = model
            maxfs = i
...
joblib.dump(clf_best, 'bestmodelKNNFS0117(1).pkl')
