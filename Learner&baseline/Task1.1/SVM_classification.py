...
path = 'D:\\3nd Semester\\Semantic Relation Extraction\\MyCode\\Multi_class\\'
# seperate data into feature set and targetset
X = pd.read_csv(path+'ModelInput.csv', sep=',',
                usecols=['positionInSentence_x', 'wordsInSentence_x', 'wordLen_x','relativePosition_x', 'positionInSentence_y',
                         'wordsInSentence_y', 'wordLen_y','relativePosition_y'], encoding='gbk')
y = pd.read_csv(path+'ModelInput.csv', sep=',',
                usecols=['class_index'], encoding='gbk')
dataset = pd.read_csv(path+'ModelInput.csv', sep=',',
                      usecols=['class_index', 'positionInSentence_x', 'wordsInSentence_x', 'wordLen_x','relativePosition_x',
                               'positionInSentence_y', 'wordsInSentence_y', 'wordLen_y','relativePosition_y'], encoding='gbk')
...
for score in scores:
    ...
    clf = GridSearchCV(svm.LinearSVC(),tuned_parameters, cv=2,
                       scoring='f1_weighted')
