...
#update 1106 try stratified k fold
...
skf = StratifiedKFold(n_splits=2) #set k
#RelationSet=['USAGE','RESULT','MDEL-FEATURE','PART_WHOLE','TOPIC','COMPARE']
#stratefied k-fold validation, for each iteration,generate train/test set and implement NB
#get the relation set
with open(dir+fn,'r') as postagging:
    reader = csv.DictReader(postagging)
    RelationSet = [row['relation'] for row in reader]
#print(RelationSet)
#get train, test data set by stratified k_fold
for train, test in skf.split(dataset,RelationSet):
    train_set=[]
    test_set=[]
    for i in train:
...
if os.path.exists("D:\\3nd Semester\\Semantic Relation Extraction\\MyCode\\output_skfold.csv"):
    os.remove("D:\\3nd Semester\\Semantic Relation Extraction\\MyCode\\output_skfold.csv")
fw = codecs.open("D:\\3nd Semester\\Semantic Relation Extraction\\MyCode\\output_skfold.csv","a+","UTF-8" )
